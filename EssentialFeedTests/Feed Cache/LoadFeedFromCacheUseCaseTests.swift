...
//  EssentialFeedTests
//
//  Created by Mazvydas Katinas on 12/09/2023.
//
 ...
 class LoadFeedFromCacheUseCaseTests: XCTestCase {
     ...
     private func expect(_ sut: LocalFeedLoader, toCompleteWith expectedResult: LocalFeedLoader.LoadResult, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
         ...
         sut.load { recievedResult in
             ...
             case let (.success(expectedImages), .success(recievedImages)):
                 XCTAssertEqual(expectedImages, recievedImages, file: file, line: line)
             case let (.failure(expectedError as NSError), .failure(recievedError as NSError)):
                 XCTAssertEqual(expectedError, recievedError, file: file, line: line)
             default:
